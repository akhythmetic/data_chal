...
knn.pred.final <- knn(train = x_train, test = x_test, cl = y_train, k = k_optimal, prob=TRUE)

probabilité <- ifelse(knn.pred.final == "1", attr(knn.pred.final, "prob"), 1 - attr(knn.pred.final, "prob"))
#Pour créer la courbe ROC
roc_obj <- roc(y_test, probabilité)
plot(roc_obj, main = paste("Courbe ROC pour k-NN (k =", k_optimal, ")"))
auc_value <- auc(roc_obj)
legend("bottomright", legend = paste("AUC =", round(auc_value, 3)), bty = "n", col = "red")

#Création du dataframe pour télécharger en csv
